...
- `main.css` scans `components/`, `layouts/` and `pages/` only. A directory left off that list gets no utilities and cannot reach the production stylesheet, which is what keeps `src/dev/inventory` (the dev-only `/inventory` page) out of a build.
